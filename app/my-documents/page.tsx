          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex items-center gap-6 w-full md:w-auto">
              ...
              <div className="relative" ref={filterRef}>
                ...
                {filterOpen && (
                  <div className="absolute right-0 top-12 w-48 bg-white rounded-xl shadow-lg border border-neutral-100 p-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="border-b border-[#D0D5DD] bg-[#F9FAFB] text-left">
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Document</th>
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Period</th>
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Date</th>
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Size</th>
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Status</th>
                  <th className="py-2.5 px-6 text-[16px] font-normal text-[#111827]">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((doc) => (
                  <tr key={doc.id} className="border-b border-[#D0D5DD]">
                    <td className="py-5 px-6 text-[14px] font-normal text-[#111827]">{doc.name}</td>
                    <td className="py-5 px-6 text-[14px] text-[#111827]">{doc.period}</td>
                    <td className="py-5 px-6 text-[14px] text-[#111827]">{doc.date}</td>
                    <td className="py-5 px-6 text-[14px] text-[#111827]">{doc.size}</td>
                    <td className="py-5 px-6">
                      <span className={`px-3.5 py-2.5 rounded-full text-[12px] 2xl:text-[14px] font-normal inline-flex ${getStatusColor(doc.status)}`}>
                        {doc.status}
                      </span>
                    </td>
                    <td className="py-5 px-6">
