...
export default function EPSSubmissionDetailsPage() {
  ...
  const isRejected = id.includes('january');
  ...
  return (
    <DashboardLayout title={`EPS - ${month} ${year}`} subtitle={breadcrumb}>
      <div className={`flex-1 p-4 2xl:p-6 ${lexendDeca.className}`}>
        ...
        <Link href={`/hmrc-rti/eps-submissions`}>
        ...
        <div className="bg-white p-4 2xl:p-6 rounded-2xl">
          <div className="flex justify-between items-center md:mb-6 mb-4">
            <h2 className="md:text-[20px] text-[18px] font-medium text-[#111827]">EPS - {month} {year}</h2>
            <button onClick={handleExport} className="flex items-center gap-2 rounded-xl border border-neutral-200 bg-white md:px-4 px-3 md:py-2 py-1 md:text-[14px] text-[12px] font-medium text-neutral-700 hover:bg-neutral-50 transition-colors">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="rounded-2xl bg-[#F9FAFB] p-6">
                <div className="grid md:grid-cols-4 grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-[12px] font-medium text-[#98A2B3] md:mb-2 mb-1">Submitted Date</p>
                    <p className="text-[14px] font-semibold text-[#111827]">5 Feb 2026</p>
                  </div>
                  <div>
                    <p className="text-[12px] font-medium text-[#98A2B3] md:mb-2 mb-1">Adjustments</p>
                    <p className="text-[14px] font-semibold text-[#111827]">SMP, SSP, SAP</p>
                  </div>
                  <div>
                    <p className="text-[12px] font-medium text-[#98A2B3] md:mb-2 mb-1">HMRC response</p>
                ...
                <div>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-[#E2E8F0] bg-white">
                    <table className="w-full text-left">
                      <thead className="border-b border-[#E2E8F0] bg-[#F8FAFC] text-[#64748B] md:text-[16px] text-[14px]">
                        <tr>
                          <th className="md:px-6 px-4 md:py-4 py-2 font-medium">Error Code</th>
                          <th className="md:px-6 px-4 md:py-4 py-2 font-medium">Field</th>
                          <th className="md:px-6 px-4 md:py-4 py-2 font-medium">Message</th>
                          <th className="md:px-6 px-4 md:py-4 py-2 font-medium">Severity</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#E2E8F0] md:text-[14px] text-[12px]">
                        {errors.map((err, idx) => (
                          <tr key={idx}>
                            <td className="md:px-6 px-4 md:py-4 py-2 font-medium text-[#64748B] align-top">{err.code}</td>
                            <td className="md:px-6 px-4 md:py-4 py-2 font-medium text-[#111827] align-top">{err.field}</td>
                            <td className="md:px-6 px-4 md:py-4 py-2 text-[#64748B] md:max-w-[400px] min-w-[400px] leading-relaxed align-top">{err.message}</td>
                            <td className="md:px-6 px-4 md:py-4 py-2 align-top">{renderBadge(err.severity)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
